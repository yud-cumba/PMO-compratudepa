<template>
  <div>
    ...
    <v-row>
      <v-col>
        <transition name="bounce" v-if="chartData">
        </transition>
      </v-col>
      <v-col>
  ...
  computed: {
      ...
      const reasons = [
        {
          ...
          function: () => {
            this.asesorByWhatsApp(this.project.project_whatsapp);
          },
        },
        {
          icon: handHeart,
    ...
    chartData() {
      if (this.project.precio_m2) {
        ...
        // const mediaDistrict = (
        //   this.project.val_price1
        //   - this.project.diferencia_distrito * this.project.val_price1
        // ).toFixed(2);
        // // eslint-disable-next-line max-len
        // const mediaNear = (
        //   this.project.val_price1
        //   - this.project.diferencia_near * this.project.val_price1
        // ).toFixed(2);
        return {
          ...
          datasets: [
            {
              ...
              }: ${this.project.clase_distrito} 
              ${priceStyle(this.project.media_distrito)}`,
              backgroundColor: '#ffc107',
              data: [this.project.media_distrito, 0],
            },
            {
              label: `Precio de ${this.project.name} por m2:
              ${this.project.coin} ${priceStyle(this.project.precio_m2)}`,
              backgroundColor: 'blue',
              data: [this.project.precio_m2, 0],
            },
            {
              label: `Promedio de precios por m2 cercanos a ${
                this.project.name
              } en 5km : ${this.project.clase_near} 
              ${priceStyle(this.project.media_near)}`,
